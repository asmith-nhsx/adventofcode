...
def appendPoint(row, col, data, points):
    if (row >= 0 and row < len(data)):
        if (col >= 0 and col < len(data[row])):
            points.append((row,col,int(data[row][col])))
...
def fill(point, data):
    ...
    filled = {}
    while len(points) > 0:
        row, col, cur = points.pop()
        if (cur != 9 and (row, col) not in filled):
            filled[(row, col)] = True
            appendPoint(row-1, col, data, points)
            appendPoint(row+1, col, data, points)
            appendPoint(row, col-1, data, points)
            appendPoint(row, col+1, data, points)
    return len(filled)
